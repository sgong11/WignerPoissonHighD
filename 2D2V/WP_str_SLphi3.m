...
%   leading-mode conservation correction.
%   The configuration includes:
%   - Problem selection (Landau damping or two-stream instability)
...
prob      = 2;  % 1 = weak Landau, 2 = strong Landau, 3-4 = two-stream
...
if test_type == 1 % single time evolution
    ...
    for l = 1:length(N_c)  
        ...
        while t(n) < T
            ...
            [f_ht{n+1},elec_mod] = HT_Conserve_correction_leading(f_ht_temp, h_mesh, v_modes, x_modes,v, mass(1), momentum(1,:), energy(1), curr_mass, curr_mom, curr_energy, phixm_ht,base_rel_eps,tol_ratio, min_rank, max_rank,kx);
    ...
    % === Finalize and save ===
        ...
        save(['LeadingSL_' name_ '_' num2str(N_mesh(1)) '_CFL' num2str(CFL) '_T' num2str(T) '_H' num2str(H) '.mat'])
    end
elseif test_type == 2
    H = 1;
    % === Simulation settings ===
    ...
    CFL     = 50.;                       % CFL number
    n_ghost = ceil(CFL) + 10;
    N_c = 32*[2,4,8,16,32,64,128]; % Mesh sizes for refinement study
    for l = 1:length(N_c)  
        N_mesh  = N_c(l)*ones(1,4);            % Grid size in all 4 dimensions [Nx Nvx Ny Nvy] 
                                            % (the order can change if you modify x_modes and v_modes)
