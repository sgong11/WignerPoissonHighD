function htd = rand_complex(tree, rank)
%RAND Generate a random complex HTD structure with specified rank.
%
%   htd = ht.rand_complex(tree, rank)
%
%   Inputs:
...
%             .rank   : copied from input
    ...
    for d = 1: tree.orders(1)
        ...
        r = rank(node);
        n = tree.modesizes(d);
        htd.U{node} = randn(n, r) + j*randn(n, r);  % standard Gaussian
    end
    ...
    for i = tree.postorder_nonleaf
        ...
        r_right = rank(c(2));
        r_here  = rank(i);
        htd.B{i} = randn(r_left, r_right, r_here) + j*randn(r_left, r_right, r_here);
    end
end